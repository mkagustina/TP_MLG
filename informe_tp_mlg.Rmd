...
\begin{itemize}
  ...
  \item \textbf{Presión arterial}: presión diastólica, clasificada en dos niveles:
  \begin{itemize}
...
\beta_7\,\text{Edad5}_i
$$
Ambos modelos fueron comparados mediante la prueba de razón de verosimilitud. Dado que la diferencia en el ajuste entre el modelo con edad tratada como ordinal y el modelo con las variables dummies no resultó estadísticamente significativa, se concluye que la relación entre la edad y el logit de la probabilidad de diabetes puede considerarse lineal ($p-value = 0.6408$). Por ende, se mantiene el efecto lineal de la variable edad en el modelo.
...
\begin{itemize}
  \item $\text{Glucosa}_i = 
  \begin{cases}
    1 & \text{si } \text{Glucosa} > 100 \\
    0 & \text{si } \text{Glucosa} \leq 100
  \end{cases}$
  
  \item $\text{DPF}_i = 
  \begin{cases}
    1 & \text{si } \text{Diabetes Pedigree Function} > 0.5 \\
    0 & \text{si } \text{Diabetes Pedigree Function} \leq 0.5
  \end{cases}$  

  \item $\text{Obesidad}_i = 
  \begin{cases}
    1 & \text{si } \text{IMC} \geq 30 \\
    0 & \text{si } \text{IMC} < 30
  \end{cases}$
\end{itemize}
...
\beta_3\,\text{DPF}_i + \beta_4\,\text{Obesidad}_i + \beta_5\,\hat{\eta}_i^2 \quad i=\overline{1,332}
\end{aligned}
$$
...
```{r, results='hide'}
pchisq(modelo$deviance, modelo$df.residual, ncp = 0, lower.tail = FALSE,log.p=FALSE) # esto es con deviance habria que hacer el de hosmer y lemeshow
...
plot(roc_train, col = "aquamarine3", lwd = 2)
